...
public:
    MenuButton(sf::Vector2f position, sf::Vector2f size, sf::Color color, sf::Font& font, const std::string buttonText = "", std::function<void()> action = []{}) : 
      image(sf::Vector2u(size), color), texture(image), sprite(texture), font(font), text(font, buttonText), action(action), originalColor(color) {
        ...
        text.setPosition({ position.x + (size.x / 2.f) - (textsize.x / 2.f),position.y });
    }
    ...
    void setPosition(sf::Vector2f position) {
        ...
        text.setPosition({ position.x + (spritesize.x / 2.f) - (textsize.x / 2.f),position.y + (spritesize.y / 2.f) - (textsize.y / 2.f) });
    }
    ...
    void setText(const std::string buttonText) {
        ...
        sf::Vector2f textsize = text.getGlobalBounds().size;
        text.setPosition({ getPosition().x + (spritesize.x / 2.f) - (textsize.x / 2.f),getPosition().y + (spritesize.y / 2.f) - (textsize.y / 2.f) });
    }
        ...
        for (int i = 0; i< lim; i++) {
            ...
            buttons.push_back(
                std::make_unique<MenuButton>(
                    sf::Vector2f(10.f,10.f+i*60.f),
                    sf::Vector2f(1260.f,60.f),
